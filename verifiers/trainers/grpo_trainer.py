...
class GRPOTrainer(Trainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.vllm_client = VLLMClient(host=host, port=port, connection_timeout=args.vllm_server_timeout)
    ...
    def get_train_dataloader(self):
        ...
        self._async_dataloader = AsyncDataLoaderWrapper(dataloader, buffer_size=max(5, self.num_batches_ahead * 2))
    ...
    def _move_model_to_vllm(self):
        ...
        while is_generating:
            time.sleep(0.5)
            self.logger.info("Waiting for background batch generation to complete before weight syncing.")
        ...
        if self._step % generate_every == 0 or self._buffered_inputs is None:
            ...
            for i in range(process_slice.start, process_slice.stop):
                ...
                completion_ids_list.append(
                    torch.tensor(broadcast_data["completion_ids"][i], device=self.accelerator.device)
                )
                completion_mask_list.append(
                    torch.tensor(broadcast_data["completion_mask"][i], device=self.accelerator.device)
                )
            ...
            completion_ids = pad(
                completion_ids_list, padding_value=self.processing_class.pad_token_id, padding_side="right"
            )  # type: ignore
    ...
    def _log_reward_metrics_primary(
        self, mode: str, all_reward_dict: Dict[str, Any], all_rewards: torch.Tensor, generation_batch_size: int
    ) -> None:
        """
    ...
    def _log_completion_metrics_primary(
        ...
        for comp_ids, comp_mask in zip(all_completion_ids, all_completion_mask):
            has_eos = any(
                token == self.processing_class.eos_token_id for token, mask in zip(comp_ids, comp_mask) if mask
            )  # type: ignore
            if has_eos:
