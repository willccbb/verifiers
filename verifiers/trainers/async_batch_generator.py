...
class AsyncBatchGenerator:
    ...
    def start(self):
        ...
        self.worker_thread = threading.Thread(target=self._generation_worker, daemon=True, name="AsyncBatchGenerator")
    ...
    def _generation_worker(self):
        ...
        self.client = AsyncOpenAI(
            ...
            http_client=httpx.AsyncClient(
                limits=httpx.Limits(
                    max_connections=self.client_config["http_client_args"]["limits"]["max_connections"]
                ),
    ...
    async def _evaluate_async(self, num_samples: int = -1) -> GenerateOutputs:
        ...
        results = await self.env.a_generate(
            inputs, client=self.client, model=self.model_name, sampling_args=self.sampling_args
        )
        return results
        ...
        def run_evaluation():
            ...
            async def run_eval():
                try:
                    ...
                    # Run generation on eval dataset
                    results = await self.env.a_generate(
                        inputs, client=eval_client, model=self.model_name, sampling_args=self.sampling_args
                    )
