...
def view_sections(page_id: str) -> list[dict]:
    ...
    if not sections:
        sections.append({"section_id": f"{page_id}:full", "section_name": "Full Page", "start_line": 0})
...
vf_env = vf.ToolEnv(dataset=dataset, system_prompt=system_prompt, tools=tools, max_turns=10, max_concurrent=512)

judge_client = OpenAI(base_url="http://0.0.0.0:8008/v1", api_key="EMPTY")
judge_model = "Qwen/Qwen2.5-7B-Instruct"
judge_rubric = JudgeRubric(judge_client=judge_client, judge_model=judge_model, parser=vf_env.parser)
