...
class VLLMClient(AsyncOpenAI):
    ...
    def __init__(
        self, host: str = "0.0.0.0", port: int = 8000, group_port: int = 51216, connection_timeout: float = 0.0
    ):
    ...
    def init_communicator(self):
        ...
        try:
            response = self.session.post(
                url, json={"host": self.host, "port": self.group_port, "world_size": world_size}
            )
        except Exception as e:
