...
class Environment(ABC):
    ...
    def format_prompt(
        self, prompt_str: str, system_prompt: str | None = None, few_shot: List[ChatMessage] | None = None
    ) -> List[ChatMessage]:
        messages = []
        if system_prompt:
    ...
    def format_dataset(
        ...
        if answer_key == "answer":
            ...
            return dataset.map(lambda x: {"prompt": format_prompt_fn(x[question_key]), "answer": x[answer_key]})
    ...
    async def get_model_response(
        ...
        **kwargs,
        ...
        if message_type == "chat":
            ...
            response = await client.completions.create(model=model, prompt=prompt, **sampling_args)
            return response
...
Model copies with swapped templates are available here: https://huggingface.co/collections/willcb/qwen3-68434f4883925bfdb4570ee5"
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        results = self.generate(inputs, client, model, sampling_args, **kwargs)
        return results
