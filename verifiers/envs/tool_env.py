...
from verifiers import Message, Messages, MultiTurnEnv, RewardFunc, State, ToolRubric, XMLParser
...
class ToolEnv(MultiTurnEnv):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(system_prompt=formatted_prompt, parser=parser, rubric=rubric, max_turns=max_turns, **kwargs)
    ...
    def call_tool(self, tool_json: str, max_chars: int = 1024, **kwargs) -> str:
        ...
        try:
            ...
            if isinstance(tool_args, str):
                tool_schema = next(
                    (schema["args"] for schema in self.tool_schemas if schema["name"] == tool_name), None
                )
                return (
                    f"Error: Arguments for {tool_name} must be a JSON object with schema {tool_schema}, not a string."
                )
