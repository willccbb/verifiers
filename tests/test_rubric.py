...
class TestRubric:
    ...
    async def test_call_reward_func_with_var_kwargs(self):
        ...
        result = await rubric.call_reward_func(
            func=kwargs_func, prompt="test", completion="test", answer="test", state={}, task="test", info={}
        )
    ...
    async def test_call_reward_func_error_handling(self):
        ...
        result = await rubric.call_reward_func(
            func=error_func, prompt="test", completion="test", answer="test", state={}, task="test", info={}
        )
    ...
    async def test_score_rollout_single(self):
        ...
        result = await rubric.score_rollout(
            prompt="test prompt", completion="test", answer="test", state={}, task="test_task", info={}
        )
    ...
    async def test_score_rollout_with_list_completion(self):
        ...
        completion = [{"role": "user", "content": "Hello"}, {"role": "assistant", "content": "Hi there!"}]

        result = await rubric.score_rollout(
            prompt="test", completion=completion, answer="test", state={}, task="test", info={}
        )
    ...
    async def test_score_rollouts_multiple(self):
        ...
        results = await rubric.score_rollouts(
            prompts=prompts, completions=completions, answers=answers, states=states, tasks=tasks, infos=infos
        )
    ...
    async def test_score_rollouts_empty(self):
        ...
        results = await rubric.score_rollouts(prompts=[], completions=[], answers=[], states=[], tasks=[], infos=[])
