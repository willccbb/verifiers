...
from verifiers.parsers import Parser, ThinkParser, XMLParser
...
class MockAsyncOpenAI:
    ...
    def add_chat_response(self, messages, response, finish_reason="stop"):
        ...
        self.chat_completions[key] = {"content": response, "finish_reason": finish_reason}

    def add_text_response(self, prompt, response, finish_reason="stop"):
        """Add a mapped response for specific prompt."""
        self.text_completions[prompt] = {"text": response, "finish_reason": finish_reason}
    ...
    async def _handle_chat_completion(self, messages, **kwargs):
        ...
        if key in self.chat_completions:
            ...
            response_data = {"content": self.default_chat_response, "finish_reason": "stop"}
    ...
    def _handle_text_completion(self, prompt, **kwargs):
        ...
        if prompt in self.text_completions:
            ...
            response_data = {"text": self.default_text_response, "finish_reason": "stop"}
...
def sample_dataset():
    ...
    return Dataset.from_dict({"question": ["What is 2+2?", "What is the capital of France?"], "answer": ["4", "Paris"]})
...
def mock_singleturn_env_completion(mock_openai_client):
    ...
    completion_dataset = Dataset.from_dict(
        {"prompt": ["Calculate 2+2:", "Name the capital of France:"], "answer": ["4", "Paris"]}
    )
    return SingleTurnEnv(
        client=mock_openai_client,
        model="test-model",
