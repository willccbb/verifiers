...
class SimpleEnvironment(Environment):
    ...
    async def rollout(self, client, model, prompt, answer, task="default", info={}, sampling_args={}, **kwargs):
        ...
        response = await self.get_model_response(prompt=prompt, client=client, model=model, sampling_args=sampling_args)
...
class TestEnvironmentBase:
    ...
    def test_environment_initialization(self, mock_openai_client, sample_dataset):
        ...
        env = SimpleEnvironment(
            client=mock_openai_client, model="test-model", dataset=sample_dataset, parser=Parser(), rubric=Rubric()
        )
    ...
    def test_environment_with_eval_dataset_only(self, mock_openai_client, sample_dataset):
        ...
        env = SimpleEnvironment(
            client=mock_openai_client, model="test-model", eval_dataset=sample_dataset, parser=Parser(), rubric=Rubric()
        )
    ...
    def test_environment_no_datasets_raises_error(self, mock_openai_client):
        ...
        with pytest.raises(ValueError, match="Either dataset or eval_dataset must be provided"):
            SimpleEnvironment(client=mock_openai_client, model="test-model", parser=Parser(), rubric=Rubric())
    ...
    async def test_get_model_response_chat(self, mock_openai_client):
        ...
        response = await env.get_model_response(
            prompt=prompt, client=mock_openai_client, model="test-model", message_type="chat"
        )
    ...
    async def test_get_model_response_completion(self, mock_openai_client):
        ...
        response = await env.get_model_response(
            prompt=prompt, client=mock_openai_client, model="test-model", message_type="completion"
        )
    ...
    async def test_get_model_response_max_tokens_reached(self, mock_openai_client):
        ...
        prompt = [{"role": "user", "content": "Hello"}]
        response = await env.get_model_response(prompt=prompt, client=mock_openai_client, model="test-model")
    ...
    def test_sanitize_sampling_args_remote_server(self, mock_openai_client):
        ...
        sampling_args = {"temperature": 0.7, "extra_body": {"skip_special_tokens": True}}
    ...
    async def test_run_rollouts(self, mock_openai_client):
        ...
        prompts = [[{"role": "user", "content": "Hello"}], [{"role": "user", "content": "Hi"}]]
        ...
        results = await env.run_rollouts(
            client=mock_openai_client, model="test-model", prompts=prompts, answers=answers, tasks=tasks, infos=infos
        )
