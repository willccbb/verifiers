...
- `Environment` classes for encapsulating your tasks, parsers, rollout logic, and reward functions, including:
	...
	- `SmolAgentsToolEnv` for multi-turn tool use with Hugging Face [smolagents](https://huggingface.co/docs/smolagents/en/index) tools.
...
**Level 4:** Implement your own multi-turn agent environment using `ToolEnv`, `SmolAgentsToolEnv`, or `CodeEnv`:
```python
import verifiers as vf
vf_env = vf.ToolEnv(
	dataset=..., # hf Dataset with 'question' + 'answer' columns
